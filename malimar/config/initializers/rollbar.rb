if Rails.env.production?
  Rollbar.configure do |config|
    ...
    config.exception_level_filters.merge!(
      ...
      "Callback::PaymentNotification::RefundError" => "info",
      "ActiveRecord::RecordNotUnique" => "debug",
      "ActiveRecord::StatementInvalid" => ->(e) { e.message.to_s.include?("expires_at") ? "debug" : "error" }
    )
    config.scrub_fields |= %i[number cvv]
