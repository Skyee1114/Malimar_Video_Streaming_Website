...
module ActiveModel
  class ArraySerializer
    ...
    alias serializable_array serializable_object
        ...
        serializer_for(item).embedded_in_root_associations.each_pair do |type, objects|
          ...
          if hash.has_key?(type)
            case hash[type] when Hash
                              hash[type].deep_merge!(objects) { |_key, old, new| (Array(old) + Array(new)).uniq }
                            else
    ...
    def instrumentation_keys
      %i[object scope root meta_key meta each_serializer resource_name key_format context]
    end
  end
end
