  ...
  class Serializer
    ...
    class << self
      ...
      def embed(type, options = {})
        CONFIG.embed = type
        CONFIG.embed_in_root = true if EMBED_IN_ROOT_OPTIONS.any? { |opt| options[opt].present? }
        CONFIG.embed_in_root_key = options[:embed_in_root_key] if options[:embed_in_root_key].present?
      ...
      associations.each_with_object({}) do |(name, association), hash|
        if included_associations.include? name
          if association.embed_ids?
            ...
            hash = hash[association.embed_namespace] ||= {} if association.embed_namespace?
    ...
    def embedded_in_root_associations
      ...
      associations.each_with_object({}) do |(name, association), hash|
        ...
        hash.merge!(association_serializer.embedded_in_root_associations) { |_key, oldval, newval| [newval, oldval].flatten }

        next unless association.embed_in_root?

        hash = hash[association.embed_in_root_key] ||= {} if association.embed_in_root_key?
    ...
    alias serializable_hash serializable_object
